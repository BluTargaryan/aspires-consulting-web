...
const PersonnelOverlay = ({ image, name, consultationFee, bio, services, paymentDetails, socials, onClose }: PersonnelOverlayProps) => {
  ...
  return (
    ...
    >
      ...
      xl:w-[1154px] xl:mx-auto xl:px-0 xl:py-10 xl:gap-22
      ">

        <button
          onClick={onClose}
          className="self-start flex items-center gap-2 bg-text text-background px-4 py-2 border border-text border-radius-button font-medium hover:bg-hover hover:text-text hover:border-text"
        >
          <MdClose />
          Close
        </button>
 ...
 <div className="flex flex-col gap-4">
   ...
   {services && services !== '' && (
     <div className="flex flex-col gap-5 px-5 py-4 border-radius-card bg-secondary 
     xl:text-2xl">
   ...
   <div className="flex flex-col items-start  px-5 py-4 gap-5 border-radius-card bg-secondary
   xl:text-2xl">
     <span className="shrink-0 font-bold">Payment details</span>
   ...
   <div className="grid grid-cols-2 gap-2 xl:grid-cols-3">
     {socials.map(({ label, url }, i) => (
       <Button
         ...
         className="w-full bg-text text-background hover:text-text hover:bg-hover hover:border hover:border-text xl:text-xl xl:h-14 xl:px-4 xl:gap-2"
